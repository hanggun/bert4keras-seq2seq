from train_with_pretrained_encoder import seq2seq_model, de_tokenizer, en_tokenizer
...
class AutoSeq(AutoRegressiveDecoder):
    ...
    def predict(self, inputs, output_ids, states):
        token_ids = inputs[0]
        segmen_ids = np.zeros_like(token_ids)
        return self.last_token(seq2seq_model).predict([token_ids, segmen_ids, output_ids])
...
en_data = open(config.val_source_dir, 'r', encoding='utf-8').read().splitlines()
de_data = open(config.val_target_dir, 'r', encoding='utf-8').read().splitlines()
...
for en, de in pbar:
    total_len += 1
    pred = autoseq.generate(en, topk=1)
